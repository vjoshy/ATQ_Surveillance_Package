#' Simulate epidemics
#'
#' This function simulates epidemics under a Susceptible-Infectious-Removed
#' (SIR) compartment framework. The time period for simulation is 270 days
#'
#' @param df individuals data frame from household data simulation
#' @param b Spatial parameter(s) (>0) or network parameter (s) (>0)
#'  if contact network is used.
#' @param sus Susceptibility parameter (>0)
#' @param spark Sparks parameter (>=0), representing infections unexplained
#'  by other parts of the model.
#' @param num_inf number of individuals that are infected
#' initially in each catchment.
...
simepi <- function(df, b, sus, spark, num_inf, rep = 10){
  ...
  for(i in 1:rep){
    ...
    runs[[i]]<- list(XYcoordinates = SIR$XYcoordinates, inftime = SIR$inftime,
                     remtime = SIR$remtime, start = start)
  }
